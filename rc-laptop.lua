...
-- Define a spacer symbol
seperator = widget({ type = "textbox" })
seperator.text = " :: "

-- CPU monitor
cpu_monitor = widget({type = "textbox" })
vicious.register(cpu_monitor, vicious.widgets.cpu, "CPU: $1%")

-- Memory usage
mem_monitor = widget({ type = "textbox" })
vicious.register( mem_monitor, vicious.widgets.mem, "Memory: $1% ($2MB/$3MB)", 13)

-- GPU montior
gpu_monitor = widget({ type = "textbox" })
function get_nvidia_status()
    local data = io.popen("cat /proc/acpi/bbswitch | awk '{print $2}'","r")
    local line = data:read()
    data:close()
    return line
end
gpu_monitor.text = "Nvidia: "..get_nvidia_status()
gpu_monitor_timer = timer({timeout = 30 })
gpu_monitor_timer:add_signal("timeout", function()
    gpu_monitor.text = "Nvidia "..get_nvidia_status()
end)
gpu_monitor_timer:start()

-- Thermal monitor
therm_monitor = widget({ type = "textbox" })
vicious.register(therm_monitor, vicious.widgets.thermal, "Temp: $1 C", 20, "thermal_zone0")
